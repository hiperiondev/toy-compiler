...
static int writeLiteralTypeToCache(Toy_LiteralArray *literalCache, Toy_Literal literal) {
    ...
    // optimization: check if exactly this literal array exists
    ...
    for (int i = 0; i < node->fnCollection.count; i++) {
        switch (node->fnCollection.nodes[i].type) {
            case TOY_AST_NODE_VAR_DECL: {
                ...
                int identifierIndex = Toy_pushLiteralArray(&compiler->literalCache, node->fnCollection.nodes[i].varDecl.identifier); // store without duplication optimization
    ...
    switch (node->type) {
        ...
        case TOY_AST_NODE_FN_DECL: {
            ...
            for (int i = 0; i < node->fnDecl.block->block.count; i++) {
                Toy_Opcode override = Toy_writeCompilerWithJumps(fnCompiler, &(node->fnDecl.block->block.nodes[i]), NULL, NULL, -4,
                        &(node->fnDecl.block->block.nodes[i]));
                if (override != TOY_OP_EOF) { // compensate for indexing & dot notation being screwy
